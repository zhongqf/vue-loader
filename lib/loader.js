...
module.exports = function (content) {
  ...
  function getRequireComponent (parts, name) {
    ...
    parts.style.forEach(function (style, i) {
      ...
      output += getRequire('style', style, i, style.scoped, name)
    })
    ...
    if (parts.template.length) {
      output += exportPrefix + '.template = ' +
        getRequire('template', parts.template[0], 0, hasLocalStyles, name)
    }
    ...
    if (components._multiComponents_) {
      for (var key in components) {
        if (components.hasOwnProperty(key) && key !== '_multiComponents_') {
          output += getRequireComponent(components[key], key)
        }
      }
    } else {
