...
module.exports = function (content) {
  ...
  function getRequire (type, part, index, scoped, component) {
    return 'require(' +
      loaderUtils.stringifyRequest(self,
        ...
        getLoaderString(type, part, scoped, component) +
  ...
  function getRequireForImport (impt, component) {
    return 'require(' +
      loaderUtils.stringifyRequest(self,
        '-!' +
        getLoaderString('style', impt, impt.scoped, component) +
        impt.src
      ) +
    ')\n'
  }

  function getLoaderString (type, part, scoped, component) {
    var lang = part.lang || defaultLang[type]
    var rewriter = scoped ? rewriters[type] || '' : ''
      
    if (component && component.length > 0 && rewriter.length > 0) {
      rewriter += '?component=' + component
    }
  ...
  function getRequireComponent (components, name) {
    ...
    parts.styleImports.forEach(function (impt) {
      op += getRequireForImport(impt, name)
    })
